...
public class OperationRepository implements Repository<Operation> {
    ...
    public Operation save(Operation operation) {
        if (operation.getBankAccountId() != null) {
            accountRepository.findById(operation.getBankAccountId())
                    .ifPresent(operation::setBankAccount);
        }
        if (operation.getCategoryId() != null) {
            categoryRepository.findById(operation.getCategoryId())
                    .ifPresent(operation::setCategory);
        }
    ...
    public List<Operation> findAll() {
        List<Operation> allOperations = new ArrayList<>(operations.values());
        for (Operation op : allOperations) {
            if (op.getBankAccountId() != null) {
                accountRepository.findById(op.getBankAccountId())
                        .ifPresent(op::setBankAccount);
            }
            if (op.getCategoryId() != null) {
                categoryRepository.findById(op.getCategoryId())
                        .ifPresent(op::setCategory);
            }
        }
